...
export const generateService = (module: Module): boolean => {
  ...
  if (fs.existsSync(serviceFile)) {
    console.error(`❌ The ${getServiceName(config.moduleName)} service already exist.`);
    return false;
  }
  ...
  if (match) {
    ...
    if (changed) {
      ...
      console.log(`⚠️ ${config.moduleName} tags already exist.`);
    }
  } else {
  ...
  const apiEndPointFilePath = process.cwd() + '/src/utils/constants/api-end-points.ts';
